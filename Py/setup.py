...
def createMakefile():
    ...
    try:
        import pybind11
        PYBIND_INC = '-I"%s"' % pybind11.get_include()
    except ImportError:
        raise CompileError('Pybind11 is not present - python extension cannot be compiled.\n'
            'If you see this error when installing via pip, and are sure that your python environment actually '
            'has pybind11 installed, you may need to run pip with a command-line argument --no-build-isolation, '
            'and make sure that setuptools and wheel packages are also installed.')
...
"""
    ...
    def tryPythonCode(PYTHON_SO_FLAGS, PYTHON_EXE_FLAGS=[]):
        ...
        if not (runCompiler(code=PYTEST_LIB_CODE,
                flags=' '.join([PYTHON_INC, NUMPY_INC, PYBIND_INC, '-shared', '-fPIC'] + PYTHON_SO_FLAGS +
    ...
    def findPythonLibMSVC():
        ...
        if runCompiler(code=PYTEST_LIB_CODE,
            flags=' '.join([PYTHON_INC, NUMPY_INC,PYBIND_INC, '/link', '/dll', '/out:agamatest.pyd'] + PYTHON_SO_FLAGS) ):
    ...
    COMPILE_FLAGS_LIB     += ['-DHAVE_PYTHON', PYTHON_INC, NUMPY_INC, PYBIND_INC]
