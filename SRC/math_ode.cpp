...
EXP void OdeSolverDOP853::init(const double stateNew[])
{
    // copy the vector x
...
EXP double OdeSolverDOP853::doStep(double dt)
{
    static const double
...
EXP double OdeSolverDOP853::getSol(double t, unsigned int i) const
{
...
template<int NDIM>
EXP Ode2SolverGL3<NDIM>::Ode2SolverGL3(const IOde2System& _odeSystem) :
...
template<int NDIM>
EXP void Ode2SolverGL3<NDIM>::init(const double stateNew[])
{
    for(int d=0; d<NDIM; d++) {
        state[d] = stateNew[d];
...
template<int NDIM>
EXP double Ode2SolverGL3<NDIM>::getSol(double t, unsigned int i) const
{
...
template<int NDIM>
EXP void Ode2SolverGL3<NDIM>::doStep(double dt)
{
...
template<int NDIM>
EXP Ode2SolverGL4<NDIM>::Ode2SolverGL4(const IOde2System& _odeSystem) :
    BaseOde2Solver(_odeSystem)
{
    if(odeSystem.size() != NDIM * 2)
...
template<int NDIM>
EXP void Ode2SolverGL4<NDIM>::init(const double stateNew[])
{
    for(int d=0; d<NDIM; d++) {
        state[d] = stateNew[d];
        state[d+NDIM] = stateNew[d+NDIM];
...
template<int NDIM>
EXP double Ode2SolverGL4<NDIM>::getSol(double t, unsigned int i) const
{
    double T = t - time;  // expected to be <=0, i.e. interpolating on the previous timestep
    if(i < (unsigned int)NDIM)  // i-th component of x(t)
...
template<int NDIM>
EXP void Ode2SolverGL4<NDIM>::doStep(double dt)
{
