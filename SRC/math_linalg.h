...
EXP void matrixRangeCheck(bool condition);
...
*/
template<typename NumT>
struct EXP IMatrixDense: public IMatrix<NumT> {
...
*/
template<typename NumT>
struct EXP BandMatrix: public IMatrix<NumT> {
    using IMatrix<NumT>::rows;
    using IMatrix<NumT>::cols;
...
template<typename NumT>
struct EXP SparseMatrix: public IMatrix<NumT> {
    using IMatrix<NumT>::rows;
    using IMatrix<NumT>::cols;
...
template<typename NumT>
struct EXP Matrix: public IMatrixDense<NumT> {
    using IMatrix<NumT>::rows;
    using IMatrix<NumT>::cols;
...
template<typename NumT>
struct EXP MatrixView: public IMatrixDense<NumT> {
    using IMatrix<NumT>::rows;
    using IMatrix<NumT>::cols;
...
template<typename NumT>
class EXP TransposedMatrix: public math::IMatrix<NumT> {
...
template<typename NumT>
EXP bool allZeros(const std::vector<NumT>& vec)
{
...
template<typename NumT>
EXP bool allZeros(const Matrix<NumT>& mat)
{
...
template<typename NumT>
EXP bool allZeros(const IMatrix<NumT>& mat)
{
...
*/
EXP void eliminateNearZeros(std::vector<double>& vec, double threshold=2e-15);
EXP void eliminateNearZeros(     Matrix<double>& mat, double threshold=2e-15);
...
EXP void blas_dtrmv(CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag,
...
template<typename MatrixType>
EXP void blas_dgemm(CBLAS_TRANSPOSE TransA, CBLAS_TRANSPOSE TransB,
...
EXP void blas_dtrsm(CBLAS_SIDE Side, CBLAS_UPLO Uplo, CBLAS_TRANSPOSE TransA, CBLAS_DIAG Diag,
...
*/
EXP std::vector<double> solveBand(const BandMatrix<double>& bandMatrix, const std::vector<double>& rhs);
