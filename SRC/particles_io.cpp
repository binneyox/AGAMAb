...
//-------JJB's CMP compressed format ------

template<> inline std::string formatCMPParticle<coord::PosCar>(
	const ParticleArray<coord::PosCar>::ElemType& point, const units::ExternalUnits& conv)
{
	return
...
template<> inline std::string formatCMPParticle<coord::PosVelCar>(
	const ParticleArray<coord::PosVelCar>::ElemType& point, const units::ExternalUnits& conv)
{
	return
	...
	const ParticleArray<ParticleAux>::ElemType& point, const units::ExternalUnits& conv)
{
	return
			...
			utils::jjbcmp(point.first.vz / conv.velocityUnit) +
			utils::jjbcmp(point.second   / conv.massUnit    ) +
			utils::jjbcmp(point.first.stellarMass   / conv.massUnit  ) +
			utils::jjbcmp(point.first.stellarRadius / conv.lengthUnit);
}
...
EXP ParticleArrayAux readSnapshot(
    const std::string& fileName,
    ...
    const bool append)
{
    ...
    else if(tolower(fileFormat[0])=='c')
    {
	    writeSnapshotCMP(fileName, particles, unitConverter, header, time);
    }
#ifdef HAVE_UNSIO
...
template EXP void writeSnapshot(
