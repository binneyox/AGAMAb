...
*/
EXP int fcmp(double x, double y, double eps=1e-15);
...
struct EXP ScalingLin {
    double uleft, uright;
    ScalingLin(double _uleft, double _uright) : uleft(_uleft), uright(_uright) {}
};

/// cos^2(PI/2*s)=(uright-u)/(uright-uleft) so du/ds vanishes at upper end 
struct EXP ScalingCos2 {
	double uleft, uright, ubar, Du;
	ScalingCos2(double _uleft, double _uright) : uleft(_uleft), uright(_uright) {
		ubar = .5*(uleft + uright); Du = .5*(uright - uleft);
	}
};

/// .25*sin^2(PI*s)=(u-uleft)*(uright-u)/(uright-uleft)^2 so du/ds vanishes at ends
struct EXP ScalingSin2 {
	double uleft, uright, ubar, Du;
	ScalingSin2(double _uleft, double _uright) : uleft(_uleft), uright(_uright) {
		ubar = .5*(uleft+uright); Du = .5*(uright - uleft);
	}
};

/// cos^2(PI/2*s)=(uright-u)/(uright-uleft) so du/ds vanishes at upper end 
struct EXP ScalingCos2Inv {
	double Delta2, uleft, uright, ubar, Du, sleft, sright, sbar, Ds;
	ScalingCos2Inv(double _Delta2, double _uleft, double _uright) : Delta2(_Delta2), uleft(_uleft), uright(_uright) {
		ubar = .5*(uleft + uright); Du = .5*(uright-uleft);
		sleft = 1/(uleft-Delta2); sright = 1/(uright-Delta2);
		sbar =.5*(sright+sleft); Ds = .5*(sright-sleft);
	}
};

/// .25*sin^2(PI*s)=(u-uleft)*(uright-u)/(uright-uleft)^2 so du/ds vanishes at ends
struct EXP ScalingSin2Inv {
	double Delta2, uleft, uright, ubar, Du, sbar, Ds;
	ScalingSin2Inv(double _Delta2, double _uleft, double _uright) : Delta2(_Delta2), uleft(_uleft), uright(_uright) {
		ubar = .5*(uleft+uright); Du = .5*(uright-uleft);
		double sleft = 1/(uleft-Delta2), sright = 1/(uright-Delta2);
		sbar =.5*(sright+sleft); Ds = .5*(sright-sleft);
	}
};
...
template<typename Scaling> EXP double scale(const Scaling& scaling, double u);
...
template<typename Scaling> EXP double unscale(const Scaling& scaling, double s, double* duds=NULL);
...
class EXP ScaledFnc: public IFunction {
public:
...
*/
template<typename Scaling>
class EXP ScaledIntegrand: public IFunctionNoDeriv {
public:
    const Scaling scaling;  ///< the instance of the scaling transformations specifying its parameters
    const IFunction& fnc;   ///< the original function of the unscaled argument u
...
/** A wrapper providing the IFunction interface for an ordinary function with one parameter,
    e.g., exp(x) */
class EXP FncWrapper: public IFunctionNoDeriv {
...
class EXP FncProduct: public IFunction {
...
class EXP LogLogScaledFnc: public IFunction {
...
class EXP Monomial: public IFunction, public IFunctionIntegral {
    const int m;
public:
...
*/
class EXP Gaussian: public IFunction, public IFunctionIntegral {
    const double sigma;
public:
...
*/
EXP double findRoot(const IFunction& F, double x1, double x2, double relToler);
...
/** Find a local minimum of a function F(x) on the finite interval [x1,x2].
    ...
    \throw  std::invalid_argument if the endpoints are not finite or relToler is not positive.
*/
EXP double findMin(const IFunction& F, double x1, double x2, double xinit, double relToler);
...
*/
EXP void hermiteDerivs(double x0, double x1, double x2,
...
*/
EXP double integrate(const IFunction& F, double x1, double x2, double relToler,
...
/** integrate a function on a finite interval, using a fully adaptive integration routine 
    ...
    \param[out] numEval - if not NULL, output the number of function evaluations in this variable.
*/
EXP double integrateAdaptive(const IFunction& F, double x1, double x2, double relToler, 
    double* error=NULL, int* numEval=NULL);
...
*/
EXP double integrateGL(const IFunction& F, double x1, double x2, int N);
...
*/
EXP void prepareIntegrationTableGL(double x1, double x2, int N, double* coords, double* weights);
...
*/
EXP void integrateNdim(const IFunctionNdim& F, const double xlower[], const double xupper[],
